...
function changeColor() {
  ...
  if (val == 0) {
      ...
      p2id.style.backgroundColor = "yellow";
    val = !val;
  }
  else {
      p2id = document.getElementById('p2');
  ...
  p2id.style.backgroundColor = "Green";
    p1id.style.backgroundColor = "yellow";
    val = !val;
  }
}
function timer()
...
function getWords() {
  if (visited) { 
    ...
    socket.emit("hello from client", word);
  }
}
...
socket.on("hello from server", (...args) => {
  document.getElementById("word").innerHTML = "Your word is: "+args[0];
});
...
recognition.onspeechend = () => {
  ...
  if ("speechSynthesis" in window) {
    ...
    socket.emit("hello from client", lastWord);
